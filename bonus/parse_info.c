...
int		parse_resolution(char *str, t_all *all)
{
	...
	mlx_get_screen_size(all->win->mlx, &all->screenx, &all->screeny);
	all->resolution->width = ft_atoi(args[1]);
	all->resolution->height = ft_atoi(args[2]);
	i = -1;
	if (all->resolution->width > all->screenx || ft_strlen(args[1]) > 5)
		all->resolution->width = all->screenx;
	if (all->resolution->height > all->screeny || ft_strlen(args[2]) > 5)
		all->resolution->height = all->screeny;
	while (args[++i])
		free(args[i]);
	free(args);
	return (1);
...
int		parse_textures(char *str, t_all *all, int code)
{
	...
	if (code == 6)
		return (path_tex(&all->path->floor, str + 3));
	return (1);
}
...
int		find(char *str, t_all *all)
{
	...
	if (ft_strnstr(str, "FL", 5) == str)
		return (parse_textures(str, all, 6));
